...
# LibUbox), then put it there instead.

macro(LIBUBOX_COMPUTE_SOURCES _framework)
...
macro(LIBUBOX_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(LIBUBOX_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(LIBUBOX_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            LIBUBOX_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(LIBUBOX_WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        get_filename_component(_basename ${_file} NAME_WE)
        get_filename_component(_path ${_file} PATH)

        if (NOT _file MATCHES "${DERIVED_SOURCES_LIBUBOX_DIR}")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("DerivedSources" REGULAR_EXPRESSION "${DERIVED_SOURCES_LIBUBOX_DIR}")
endmacro()

macro(LIBUBOX_FRAMEWORK_DECLARE _target)
...
macro(LIBUBOX_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like LIBUBOX_FRAMEWORK and LIBUBOX_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support LIBUBOX_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_LIBUBOX_TARGET _target_logical_name _target_cmake_name)
...
macro(_LIBUBOX_TARGET_ANALYZE _target)
    if (ClangTidy_EXE)
...
macro(LIBUBOX_FRAMEWORK _target)
    _LIBUBOX_TARGET(${_target} ${_target})
    _LIBUBOX_TARGET_ANALYZE(${_target})
...
# FIXME Move into LIBUBOX_FRAMEWORK after all libraries are using this macro
macro(LIBUBOX_FRAMEWORK_TARGET _target)
    ...
    add_library(LibUbox::${_target} ALIAS ${_target}_PostBuild)
endmacro()

macro(LIBUBOX_EXECUTABLE _target)
    _LIBUBOX_TARGET(${_target} ${_target})
    _LIBUBOX_TARGET_ANALYZE(${_target})
...
macro(LIBUBOX_WRAP_EXECUTABLE _target)
    ...
    _LIBUBOX_TARGET(${_target} ${_wrapped_target_name})
    _LIBUBOX_TARGET_ANALYZE(${_wrapped_target_name})
...
macro(LIBUBOX_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # LibUbox2 generate-forwarding-headers script.
...
macro(LIBUBOX_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    LIBUBOX_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                LIBUBOX_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(LIBUBOX_MAKE_FORWARDING_HEADERS framework)
...
function(LIBUBOX_COPY_FILES target_name)
    set(options FLATTENED)
...
macro(LIBUBOX_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(LIBUBOX_DEBUG_DUMP_VARIABLES)
...
macro(LIBUBOX_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(LIBUBOX_POPULATE_LIBRARY_VERSION library_name)
...
macro(LIBUBOX_CREATE_SYMLINK target src dest)
